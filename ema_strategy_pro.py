...
# ════════════════════════════════════════════════════════════════
#  DATA FETCHING  (cached 30s)
# ════════════════════════════════════════════════════════════════
@st.cache_data(ttl=30, show_spinner=False)
def fetch_ohlcv(symbol: str, timeframe: str, limit: int = N_BARS) -> pd.DataFrame | None:
...
def main() -> None:
    ...
    with hcol1:
        st.markdown(
            ...
            f'{coin} · {tf.upper()} · HTF FILTER: {HIGHER_TF.get(tf, "?").upper()}</p>',
            unsafe_allow_html=True,
        )
    with hcol2:
    ...
    # ── Fetch data ───────────────────────────────────────────────
    status = st.empty()
    status.info("⏳ Fetching market data from Binance…")

    df_main = fetch_ohlcv(coin, tf)
    htf     = HIGHER_TF.get(tf, "4h")
    df_htf  = fetch_ohlcv(coin, htf, limit=250)

    status.empty()

    if df_main is None or df_main.empty:
        st.error(
            "⚠ Could not fetch data. Possible causes:\n"
            "- No internet connection\n"
            "- Binance API unavailable in your region (use VPN)\n"
