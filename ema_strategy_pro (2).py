...
def _lay(**overrides):
    """Return base layout merged with per-chart overrides — never duplicates keys."""
    out = dict(_BASE_LAYOUT)
    out.update(overrides)
    return out

# Keep _LAY alias so existing calls still work (used in price + equity charts)
_LAY = _lay(
...
def build_regime_chart(df):
    ...
    # One coloured line segment per regime level
    for rv in sorted(REGIME_COLOR.keys()):
        mask = df["regime"] == rv
        if not mask.any():
            continue
        fig.add_trace(go.Scatter(
            x=df.index, y=df["regime"].where(mask),
            mode="lines",
            line=dict(color=REGIME_COLOR[rv], width=3),
            name=REGIME_LABEL[rv],
            connectgaps=False,
            showlegend=True,
        ))

    # Horizontal dashed reference lines via simple 2-point Scatter
    for rv, rc in REGIME_COLOR.items():
        fig.add_trace(go.Scatter(
            x=[df.index[0], df.index[-1]], y=[rv, rv],
            mode="lines",
            line=dict(color=rc, width=0.7, dash="dot"),
            opacity=0.5,
            showlegend=False,
            hoverinfo="skip",
        ))
...
def main():
    ...
    with tab_regime:
        ...
        with rc2:
            ...
            if "regime" in df.columns:
                ...
                fig_rg.update_layout(_lay(
                    height=220,
                    margin=dict(l=0, r=50, t=10, b=0),
                    xaxis=dict(title="% of bars", gridcolor="#111e33"),
                    yaxis=dict(gridcolor="#111e33"),
                    showlegend=False,
                ))
    ...
    with tab_bt_tab:
        if stats:
            ...
            fd.update_layout(_lay(
                height=240,
                margin=dict(l=0, r=0, t=10, b=0),
                xaxis=dict(title="P&L ($)", gridcolor="#111e33"),
                yaxis=dict(title="Count",   gridcolor="#111e33"),
            ))
